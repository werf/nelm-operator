...
KIND_CLUSTER ?= nelm-operator-test-e2e
...
deploy-source-controller: ## Deploy Flux source-controller with RBAC (for dev/testing).
	...
	"$(KUBECTL)" apply -f https://github.com/fluxcd/source-controller/releases/download/$(SOURCE_CONTROLLER_VERSION)/source-controller.crds.yaml
	"$(KUBECTL)" apply -f https://github.com/fluxcd/source-controller/releases/download/$(SOURCE_CONTROLLER_VERSION)/source-controller.deployment.yaml -n flux-system
	"$(KUBECTL)" create clusterrolebinding source-controller-admin \
		--clusterrole=cluster-admin --serviceaccount=flux-system:default 2>/dev/null || true
	@echo "Waiting for source-controller to be ready..."
	"$(KUBECTL)" -n flux-system rollout status deploy/source-controller --timeout=60s

.PHONY: undeploy-source-controller
undeploy-source-controller: ## Remove Flux source-controller.
	"$(KUBECTL)" delete -n flux-system deploy/source-controller --ignore-not-found=true
...
.PHONY: dev-deploy
dev-deploy: docker-build kind-load deploy ## Build image, load into kind, and deploy.
